...
use App\Mod_sishab\OfertaPublica\ResumoContratosProtocolo;
use App\Mod_sishab\OfertaPublica\RestricaoContrato;
use App\Mod_sishab\OfertaPublica\Substituicao;
use App\Mod_sishab\OfertaPublica\Beneficiario;
use App\Mod_sishab\OfertaPublica\PagamentosContrato;
use App\Mod_sishab\OfertaPublica\ResumoPagamentos;
...
use App\Mod_sishab\OfertaPublica\PagamentosContratosParcela;
...
class ContratoController extends Controller
{
    ...
    $remessaDevolucao = DevolucaoContratos::join('tab_remessas_devolucao','tab_remessas_devolucao.id','=','tab_devolucao_contratos.remessa_devolucao_id')                                                    
        ...
        return view('views_sishab.oferta_publica.dadosContrato',compact('contrato','restricoes','substituicoes','beneficiario','pagamentos','resumoPagamentos','protocolo','devolucoes','remessaDevolucao'));    
    } 
    ...
    public function contrato(Request $request){
       ...
       if($request->num_oferta){
        $whereContrato[] = ['num_oferta', $request->num_oferta];
            ...
            $restricoes->load('tipoRestricao');
            return view('views_sishab.oferta_publica.dadosContrato',compact('contrato','restricoes','substituicoes','beneficiario','pagamentos','resumoPagamentos',
    ...
    public function execucaoObras(Request $request){
        ...
        if(!$instituicao && !$estado && !$municipio){
            $subtitulo1 = 'Brasil';
            ...
            if($instituicao){
                ...
                if($estado && $municipio){
                    ...
                    $subtitulo2 = $estado->txt_uf;                  
        
                }
            }else{
                if($estado && $municipio){
                    $subtitulo1 = $municipio->ds_municipio .'/'.$estado->txt_uf;        
                }else if($estado && !$municipio){
        ...
        if(($request->municipio) && ($request->estado)){       
            return view('views_sishab.oferta_publica.execucaoObrasMunicipio',compact('execucaoObra','totalizadores2009','totalizadores2012','instituicao','estado','subtitulo1',
            'subtitulo2', 'subtitulo3'));            
        }else{  
            return view('views_sishab.oferta_publica.execucaoObrasUF',compact('execucaoObra','totalizadores2009','totalizadores2012','instituicao','estado','subtitulo1',
            'subtitulo2', 'subtitulo3'));
        }    
    }
    ...
    public function filtroExecucaoObras(){
        //return 'teste';
        return view('views_sishab.oferta_publica.filtro_execucao_obras');
    }

    public function filtro_contratos_if(){
        $instituicoes = Instituicao::orderBy('txt_nome_if')->get();
        return view('views_sishab.oferta_publica.filtroContratosIf',compact('instituicoes'));    
    } 
    ...
    public function contratos_instituicao(Request $request){
        ...
        return view('views_sishab.oferta_publica.contratosIF',compact('instituicao','contratos', 'qtd_contratadas','qtd_restricao','qtd_devolvido','qtd_entregue','qtd_concluida',
