<template>
   <div class="form-group">   
        <div class="row" v-if="!(estado || (modalidade) || (tipoLiberacao) || (mesSolicitacao) || (posicao_de) || (posicao_ate) || (mesLiberacao) ||(posicao_deLib) || (posicao_ateLib))">
            ...
            <div class="col-sm-12">
                <input type="text" class="form-control" id="num_apf" name="num_apf" placeholder="Digite o APF sem o ponto e o hífen. Ex.: 123456" v-model="apfdigitado">
            </div>
        </div>
        ...
        <div class="row" v-if="!apfdigitado">
            <div class="column col-xs-12 col-sm-4">
                <label for="mesLiberacao">Mês de Liberação</label>           
                <select 
                    id="mesLiberacao"
                    class="form-control" 
                    name="mesLiberacao"   
                    :disabled="buscandoMesLib"
                    @change="onChangeMesLiber"
                    v-model="mesLiberacao">
                    <option value="" v-text="textoEscolhaMes"></option>
                    <option v-for="item in mesesLiberacao" v-text="item.mes_liberacao +'-' + item.ano_liberacao" :value="item.num_mes_liberacao +'-' + item.ano_liberacao" :key="item.num_mes_liberacao +'-' + item.ano_liberacao"></option>
                </select>
            </div>
            <div class="column col-xs-12 col-sm-4">
                <label for="posicao_deLib">De</label>
                <select 
                    id="posicao_deLib"
                    class="form-control" 
                    name="posicao_deLib" 
                    :disabled="buscandoLiberacoes"
                    @change="onChangePosicaoLib"
                    v-model="posicao_deLib"
                    >
                    <option value="" v-text="textoEscolhaPosicaoDeLib"></option>
                    <option v-for="liberacao in posicoes_deLib" v-text="liberacao.dte_posicao_formatada" :value="liberacao.dte_liberacao" :key="liberacao.dte_liberacao"></option>
                </select>        
            </div>
            <div class="column col-xs-12 col-sm-4">
                <label for="periodos_ateLib">Até</label>
                <select 
                    id="posicao_ateLib"
                    class="form-control" 
                    name="posicao_ateLib" 
                     :disabled="posicao_deLib == '' || buscandoPosicaoAteLib"
                    v-model="posicao_ateLib"
                    >
                    <option value="" v-text="textoEscolhaPosicaoAteLib"></option>
                     <option v-for="posicao in posicoes_ateLib" v-text="posicao.dte_posicao_formatada" :value="posicao.dte_liberacao" :key="posicao.dte_liberacao"></option>
                </select>        
            </div>
