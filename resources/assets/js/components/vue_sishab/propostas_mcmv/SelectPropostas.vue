<template>
   <div class="form-group-border">  
        ...
        <div class="row">
            ...
            <input v-if="selecao || estado || modalidade" class="btn btn-lg btn-info btn-block" type="submit" value="Pesquisar">      
...
<script>
    export default {
        ...
        mounted() {
            ...
            //Implementação correta após arrumar tabela dos seleção
            axios.get(this.url + '/api/selecao/').then(resposta => {
                this.textoEscolhaSelecao = "Escolha uma seleção";
                this.buscandoEstado = false;
                this.selecoes = resposta.data;                        
            }).catch(error => {
                console.log(error);
