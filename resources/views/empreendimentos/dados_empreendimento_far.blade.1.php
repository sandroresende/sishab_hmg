...
@section('content')
    ...
    <div id="content">
        ...
        @if($operacao->txt_situacao_obra=='Concluída')
            @if($operacao->qtd_uh_financiadas==$operacao->qtd_uh_entregues)
            <div class="alert alert-success text-center" role="alert">
                Entregue
            </div>   
            @else
                @if(($operacao->txt_situacao_obra == 'Concluída') && ($operacao->prc_obra_realizado<95))
                ...
                @endif                        
            @endif                       
        @elseif(($operacao->txt_situacao_obra=='Normal') || ($operacao->txt_situacao_obra=='Em Andamento'))
            <div class="alert alert-secondary text-center" role="alert">
                {{$operacao->txt_situacao_obra}}
            </div>  
        @elseif($operacao->txt_situacao_obra=='Entregue')
            <div class="alert alert-success text-center" role="alert">
                {{$operacao->txt_situacao_obra}}
            </div>  
        @elseif(($operacao->txt_situacao_obra=='Atrasada') || ($operacao->txt_situacao_obra=='Não Iniciada') || ($operacao->txt_situacao_obra=='Obra Retomada'))
            <div class="alert alert-warning text-center" role="alert">
                {{$operacao->txt_situacao_obra}}
            </div>  
        @elseif(($operacao->txt_situacao_obra=='Cancelada/Distratada') || ($operacao->txt_situacao_obra=='Paralisada'))
            <div class="alert alert-danger text-center" role="alert">
                {{$operacao->txt_situacao_obra}}
            </div>  
        @else
            @if($operacao->prc_obra_realizado==0)
                <div class="alert alert-danger text-center" role="alert">
                    Não Iniciada
                </div>   
            @elseif(($operacao->prc_obra_realizado>0) && ($operacao->prc_obra_realizado<=100))
                <div class="alert alert-default text-center" role="alert">
                    Em Execução
                </div>   
            @elseif($operacao->qtd_uh_financiadas==$operacao->qtd_uh_entregues)
                <div class="alert alert-success text-center" role="alert">
                    Entregue
                </div>   
            @else
